...
function dsf_create_webshop_upper_menu_cache($parent_id=0, $spacer='', $loop=''){
						...
						// standard link routines
									...
									if ((int)$item_clickable == 1){
										...
										if ($menu_type == 2 || $loop == 'yes'){ // we are a dropdown, we can therefore go down upto two levels
											...
											$pline .= dsf_create_webshop_upper_menu_cache($id, $spacer . '   ', 'yes');
										...
										if (is_array($return_value1)){
											...
											$dis_query = dsf_db_query("select * from distributors order by sort_order");
...
//$tmp_cache_file = strtolower(DS_FS_WEBSHOP . 'cache/' . CONTENT_CACHE_PREFIX . CONTENT_COUNTRY . '_' . LANGUAGE_URL_SUFFIX . '_upper_menu_cache.php');

$tmp_cache_file = "cache/gb_en_upper_menu_cache.php";
...
// create the menu
 $new_menu = dsf_create_webshop_upper_menu_cache(0);
