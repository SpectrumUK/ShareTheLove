...
function dsf_create_webshop_upper_menu_mobile_array($parent_id=0, $spacer='', $loop='', $array_level = ''){

$line = "" . "\n";

if (strlen($array_level) < 1){
	// never ran before therefore we start the array.
	$array_level = '\$mobile_upper_menu_array';
}
									...
									if ($customer_status == 1){ // any user
										...
										// look for children
												if ((int)$menu_type == 2 ){ // dropdown only
													...
													$pline .= dsf_create_webshop_upper_menu_mobile_array($id, $spacer . '   ', 'yes', $array_level . "[" . $id . "]['children']");
												}
...
$tmp_cache_file = strtolower(DS_FS_WEBSHOP . 'cache/' . CONTENT_CACHE_PREFIX . CONTENT_COUNTRY . '_' . LANGUAGE_URL_SUFFIX . '_upper_menu_cache_mobile.php');
...
// create the menu
 $new_menu = dsf_create_webshop_upper_menu_mobile_array(0);
