...
?>
	<h1><?php echo ds_strtoupper(TRANSLATION_ARTICLE_THREE_TITLE);?></h1>
    ...
    <div class="dsHAO">
		...
		*/	
				...
				if (strlen($item['url']) > 1 && $item['url_window'] == 'true'){ // we have a url and open in new window
...
<script type="text/javascript">
	function showArts(dssid, dspid, dslvl){
							...
							$.ajax({
							method: "get",url: "article_three_ajax.php",data: "id="+dssid + "&pid=" + dspid + "&lvl=" + dslvl +"&tme=" + <?php echo time();?>,
			...
			if (isset($dsv_nested_menu) && is_array($dsv_nested_menu)){
				...
				<?php
					...
					foreach ($dsv_nested_menu as $item){
						$mcounter ++;
						if ($mcounter <=6){
							echo '<li><a href="' . dsf_article_two_url($dsv_article_id) . '#Articles" title="' . $item['name'] . '" onclick="showArts(' . $item['id'] . ',' . $item['parent_id'] . ');" class="aScroll">' . strtoupper($item['name']) . '</a></li>';
						}
					}
				?>
				</ul>
...
// display the main sections.
?>
<?php /*<div class="details" id="contentDiv">
	<div id="contentHeader">
    	<div class="breadCrumb"><?php echo $dsv_breadcrumb; ?></div>
    </div>
</div>
    <div class="clear"></div>*/?>
    <?php /*<div id="dsCRESTstrip">&nbsp;</div>*/?>
...
?>	   
	<?php /*<h1> echo $dsv_article_name;</h1> */?>
    
    <div class="dsHAO">
    	<ul>
		<?php

		// SPECIFIC CODE FOR RUSSELL SHOW ONLY 3 ITEMS THEN A MENU
		if (isset($dsv_subset_menu) && sizeof($dsv_subset_menu) > 1){
			
			// run a new procedure to show only 3 items then a list.
			$rhcounter = 0;
			...
			foreach($sub_articles as $item){
				...
				if ($rhcounter < 4){
					...
					<li<?php echo $js_link;?>>
                    	<div class="dsHAOHolder">
						<div class="dsHAOImg"><?php echo dsf_image($item['image'], $item['title'], $item['image_width'], $item['image_height']);?></div>
						<div class="dsHAOTitle"><?php echo $item['name'];?></div>
						<div class="dsHAOTxt"><?php echo $item['text'];?></div>
                        </div>
					</li>			
		...
		// now do a further LI this time to put a menu list in it.
			if (isset($dsv_subset_menu)){
				?><li>
                	<div class="dsSUBitem">
                    	<ul><?php
							foreach ($dsv_subset_menu as $item){
								
									if (isset($item['override_url']) && strlen($item['override_url']) > 3){
											echo '<li><a href="' . $item['override_url'] . '" target="_blank" title="' . $item['name'] . '">' . $item['name'] . '</a></li>';
									}else{
...
<?php	   
	...
	// start the a-z routine.
		...
		// a-z
		if (isset($a_z_array) && is_array($a_z_array)){
			...
			if (isset($dsv_nested_menu) && is_array($dsv_nested_menu)){
				...
				<?php
					...
					foreach ($dsv_nested_menu as $item){
						$mcounter ++;
						if ($mcounter <=6){
							echo '<li><a href="' . dsf_article_two_url($item['id']) . '">' . strtoupper($item['name']) . '</a></li>';
						}
					}
				?>
				</ul>
...
<div id="dsITholder">
    <div class="dsHAO">
		...
		/*
			...
			<li<?php echo $js_link;?>>
				<div class="dsHAOIMG"><?php echo dsf_image($item['image'], $item['name'], $item['image_width'], $item['image_height']);?></div>
				<div class="dsHAOTXT">
...
<div id="dsContent" class="dsARTContent">
   ...
   */
   ...
   ?>     <div class="dsAIMG">
	...
	// end additional coded 360 file programming.
	?>
    ...
    ?>
     <div class="dsAGHolder">
      <div class="dsAGTitle"><?php 
       ...
       echo ds_strtoupper($dsv_article_details['gallery']['title']);
	...
	<!-- Small Products -->				
					<div id="dsARTProSML">
						...
						<?php
							...
							if (isset($dsv_article_details['products'][$i]['awards']) && is_array($dsv_article_details['products'][$i]['awards'])){
